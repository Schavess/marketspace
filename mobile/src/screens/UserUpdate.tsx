  ...
  async function onSubmit(data: any) {
    try {
      ...
      if (response.status === 200) {
        ...
        const imageUri = response.data.avatar;
        ...
        await updateUserProfile(userToUpdated);
        ...
        Alert.alert('Erro', 'Ocorreu um erro ao atualizar a conta. Tente novamente.');
      }
    } catch (error) {
    ...
    <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
      <VStack bg={"gray.600"} borderRadius={20} pb={5} alignItems="center" >
        <Center my={10} w={'80%'} >
          ...
          <Text fontSize="xl" fontFamily="heading">Oi {user.name}!</Text>
          <Text color="gray.300" fontSize="sm" textAlign={'center'}>Atualize seus dados cadastrais.</Text>
        </Center>
        <Center w={'300'}>
          <TouchableOpacity onPress={pickImage}>
            <Image
              source={image ? { uri: image } : { uri: `${api.defaults.baseURL}/images/${user.avatar}` }}
              alt="User Photo"
              mb={5}
              width={'130px'}
              height={'130px'}
          ...
          <Controller
            ...
            defaultValue=""
          />
          {errors.oldpassword && <Text color="red.500">A Senha atual é obrigatória.</Text>}
          ...
          <Controller
            ...
            defaultValue=""
          />
          {/* {errors.password && <Text color="red.500">A Senha é obrigatória.</Text>} */}
          <Button
            title="Salvar"
            textColor='white'
            w={'full'}
